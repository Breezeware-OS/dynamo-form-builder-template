...
export default function DeleteResponseModal({
  ...
  /**
   * Deletes the response when submit btn is clicked
   */
  const deleteResponse = () => {
    BackendService.deleteUserResponse(currentResponseData?.id)
      .then(res => {
        ...
        setSeverity('success');
        getData();
        setNotificationMessage('Response Deleted Successfully.');
      })
      .catch(err => {
    ...
    <>
      ...
      <Modal open={open} onClose={closeModal} id="modal">
        <ModalTitle id="modal-title">
          <Text type="h2">Delete Response</Text>
          <IconButton
            onClick={closeModal}
            size="small"
            style={{padding: 0}}
